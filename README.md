...
trading-brain/
├── app/
│   ├── agents/             # policy、web_search、prediction、strategy、risk、execution、human_review agents
│   ├── api/                # routes.py（含 /workflow/run, /resume, /status）
│   ├── core/               # config.py
│   ├── models/             # workflow、policy、trading_state、api_state 等模型
│   └── services/           # workflow_graph、workflow_factory、web_search、market_anomaly 等服务
...
├── scripts/                # 回放/报告脚本
├── workflow_demo/          # 轻量 LangGraph demo 入口
├── static/                 # 简易 Web 前端（/ui）
├── tests/
├── main.py                 # FastAPI 入口
└── requirements.txt
...
| POST | `/mock/predict` | Mock 预测接口（主应用内置） |
| POST | `/mock/strategy` | Mock 策略接口（主应用内置） |
| POST | `/mock/execute` | Mock 执行接口（主应用内置） |
...
# Mock 预测/策略/执行接口均挂在主应用下
curl -X POST http://127.0.0.1:8000/mock/predict \
  -H "Content-Type: application/json" \
  -d '{"trading_date":"2026-05-25","market_type":"day_ahead"}'
```
